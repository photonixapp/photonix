...
def requeue_stuck_tasks(task_type, age_hours=24, max_num=8):
    # Set old, stuck or failed jobs back to Pending
    cutoff = timezone.now() - timedelta(hours=age_hours)
    for task in Task.objects.filter(type=task_type, status='S', updated_at__lt=cutoff)[:max_num]:
        task.status = 'P'
        task.save()
    for task in Task.objects.filter(type=task_type, status='F', updated_at__lt=cutoff)[:max_num]:
        task.status = 'P'
        task.save()
