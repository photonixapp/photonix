...
def main():
    ...
    for classifier in sorted(set(results_a) & set(results_b)):
        ...
        if cpu_a and cpu_b:
            # Normalise per run - result sets may have used different --runs
            runs_a = len(next(iter(a['images'].values()))['cpu_seconds'])
            runs_b = len(next(iter(b['images'].values()))['cpu_seconds'])
            cpu_a, cpu_b = cpu_a / runs_a, cpu_b / runs_b
            print(f'  cpu per pass:  {cpu_a:.2f}s -> {cpu_b:.2f}s  ({cpu_a / cpu_b:.2f}x)')
