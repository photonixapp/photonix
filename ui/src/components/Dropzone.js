...
const Dropzone = () => {
  ...
  const onDrop = useCallback(
    (acceptedFiles) => {
      ...
      axios
        .post(`/upload/?library_id=${activeLibrary.id}`, formData, {
          headers: {
      ...
      {isDragActive ? (
        <div style={{ textAlign: 'center' }}>
          <img
            src={uploadIcon}
            alt="Upload"
            style={{ filter: 'invert(0.9)', height: '100px', width: '100px' }}
          />
          <p style={{ color: 'white', fontWeight: 'bold', fontSize: 'large' }}>
            Upload to {activeLibrary.name}
          </p>
        </div>
      ) : (
        <button
