    ...
    def handle(self, *args, **options):
        ...
        try:
            while True:
                ...
                for task in Task.objects.filter(type='classify.object', status='S', updated_at__lt=timezone.now() - timedelta(hours=24))[:8]:
                    task.status = 'P'
                    task.save()
                for task in Task.objects.filter(type='classify.object', status='F', updated_at__lt=timezone.now() - timedelta(hours=24))[:8]:
                    task.status = 'P'
                    task.save()
