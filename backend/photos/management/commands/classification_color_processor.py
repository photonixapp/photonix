    ...
    def handle(self, *args, **options):
        ...
        try:
            while True:
                ...
                for task in Task.objects.filter(type='classify.color', status='S', updated_at__lt=datetime.now() - timedelta(hours=24))[:8]:
                    task.status = 'P'
                    task.save()
                for task in Task.objects.filter(type='classify.color', status='F', updated_at__lt=datetime.now() - timedelta(hours=24))[:8]:
                    task.status = 'P'
                    task.save()
